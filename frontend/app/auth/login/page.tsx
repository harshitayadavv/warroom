  ...
  return (
    <AuthLayout title="Welcome back" subtitle="Sign in to your WarRoom account">
      <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-mono)', fontSize: '10px', color: 'var(--text-muted)', letterSpacing: '0.08em' }}>
          <Link href="/auth/signup" style={{ color: 'var(--amber)', textDecoration: 'none' }}>Create account →</Link>
          <Link href="/auth/reset" style={{ color: 'var(--text-muted)', textDecoration: 'none' }}>Forgot password?</Link>
        </div>
      </form>
    </AuthLayout>
  )
...
function AuthField({ label, type, value, onChange, placeholder }: { label: string; type: string; value: string; onChange: (v: string) => void; placeholder: string }) {
  return (
    <div>
